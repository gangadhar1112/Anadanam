...
class AnadanamDetailsScreen extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    final distance =
    ref.watch(locationProvider.notifier).calculateDistance(
    ...
    return Scaffold(
        ...
        children: [
          CustomScrollView(
            ...
            slivers: [
              ...
              SliverPadding(
                // Extra space at the bottom so the map/content
                // can scroll above the bottom action bar.
      ...
      // IMPORTANT:
      // Use bottomNavigationBar instead of Positioned.
      // This gives the bottom bar a proper bounded height
      // and prevents "Cannot hit test a render box with no size".
  ...
  Widget _buildContent(
    ...
    final foodDetails =
        data['foodDetails'] as String? ?? '';

    final foodList = foodDetails
        .split(',')
        .map((e) => e.trim())
          ...
          return AlertDialog(
            ...
            content: const Text(
              'Important: Please confirm only if all food has been served. '
                  'Removing this post now will hide it from the map, and others '
                  'who might be traveling here will no longer be able to find you.',
              style: TextStyle(
                height: 1.5,
              ),
            ),
      ...
      try {
        ...
        if (context.mounted) {
          ...
          ScaffoldMessenger.of(context).showSnackBar(
            const SnackBar(
              content: Text(
                'Anadanam completed and removed.',
              ),
            ),
          );
        }
